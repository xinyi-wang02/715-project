...
From the box plot of the age distribution in each diabetes group, it is evident that the distributions do not follow a normal distribution, and the sample sizes vary significantly across the groups. Therefore, a non-parametric test is suitable for comparing the differences in medians among the various diabetes groups. Specifically, the Kruskal-Wallis test is appropriate because it is designed for comparing three or more independent groups.

Null Hypothesis (H0): The median age among test participants with diabetes, without diabetes, and with borderline cases are all equal.

Alternative Hypothesis (H1): At least one group's median age is different from the others among test participants with diabetes, without diabetes, and with borderline cases.
